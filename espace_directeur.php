...
?>
    ...
    <div class="title text-center">
            <h1 class="title-blue"><a name="member_prof" style="font-size: 60px;">Professeurs Membre</a></h1>
        </div>
        <div style="display: flex; justify-content: center; align-items: center;">
        <div style="margin-right: 30px;">
        <a data-aos="fade-right" data-aos-delay="900" href="ajouter_membre.php" class="btn btn-primary">ADD Prof</a>
        </div>
        <div style="margin-right: 30px;">
            <form method="POST" action="">
                <input type="text" name="idprof" id="idprof" placeholder="1254896" required>
                <button type="submit">Rechercher par Id</button>
            </form>
        </div>
        <div>
            <form method="POST" action="">
                <input type="text" name="nomprof" id="nomprof" placeholder="Nom du professeur" required>
    ...
    <div>
        <?php
        $idLabos = $row['id_labos'];
        $idDirecteur = $_SESSION['id'];
        // Vérifier si un formulaire de recherche a été soumis
        if (isset($_POST['idprof'])) {
            // Requête SQL pour récupérer les professeurs par ID
            $idProf = $_POST['idprof'];
            $sql = "SELECT pr.id_encadrent, pr.nom, pr.prenom, pr.id_labos, r.role
                    FROM professeur pr
                    INNER JOIN roles r ON pr.id_encadrent = r.id_encadrent
                    WHERE pr.id_labos = $idLabos AND pr.id_encadrent = $idProf AND pr.id_encadrent != $idDirecteur";
                    ...
                    if ($result && mysqli_num_rows($result) > 0) {
                        echo '<table>
                            ...
                            while ($rowprofesseur = mysqli_fetch_assoc($result)) {
                                $idProf = $rowprofesseur['id_encadrent'];
                                $sqlProjet = "SELECT COUNT(*) AS total_projet FROM projet WHERE id_encadrent = $idProf";
                                $resultProjet = mysqli_query($connexion, $sqlProjet);
                                $rowProjet = mysqli_fetch_assoc($resultProjet);
                    
                                $sqlDoctoral = "SELECT COUNT(*) AS total_doctorant FROM doctoral WHERE id_encadrent = $idProf";
                                $resultatDoctoral = mysqli_query($connexion, $sqlDoctoral);
                                $rowDoctoral = mysqli_fetch_assoc($resultatDoctoral);
                    
                                $sqlEquipe = "SELECT e.équipe FROM équipes e JOIN professeur p ON e.id_équipe=p.id_équipe WHERE id_encadrent = $idProf";
                                $resultatEquipe = mysqli_query($connexion, $sqlEquipe);
                                $rowEquipe = mysqli_fetch_assoc($resultatEquipe);
                        
                                echo '<tr>
                                    <td><a href="directeur_prof.php?id_encadrent=' . $idProf . '">' . $rowprofesseur['id_encadrent'] . '</a></td>
                                    <td>' . $rowprofesseur['nom'] . '</td>
                                    <td>' . $rowprofesseur['prenom'] . '</td>
                                    <td>' . $rowEquipe['équipe'] . '</td>
                                    <td>' . $rowProjet['total_projet'] . '</td>
                                    <td>' . $rowDoctoral['total_doctorant'] . '</td>
                                    <td><a href="afficher_prof.php?id_encadrent=' . $idProf . '">iInformation</a></td>
                                    <td><a href="modifier_prof.php?id_encadrent=' . $idProf . '">Modifier</a></td>
                                    <td><a href="supprimer_prof.php?id_encadrent=' . $idProf . '">Supprimer</a></td>
                                </tr>';
                            }
        ...
        } elseif (isset($_POST['nomprof'])) {
            // Requête SQL pour récupérer les professeurs par nom
            $nomProf = $_POST['nomprof'];
            $sql = "SELECT pr.id_encadrent, pr.nom, pr.prenom, pr.id_labos, r.role
                    FROM professeur pr
                    INNER JOIN roles r ON pr.id_encadrent = r.id_encadrent
                    WHERE pr.id_labos = $idLabos AND pr.nom = $nomProf AND pr.id_encadrent != $idDirecteur";
                    $result = mysqli_query($connexion, $sql);
                    ...
                    if ($result && mysqli_num_rows($result) > 0) {
                        ...
                        $idLabos = $row['id_labos'];
                        // Requête SQL pour récupérer les professeurs ayant le même id_labos que le directeur
                        $sql = "SELECT pr.id_encadrent, pr.nom, pr.prenom, pr.id_labos, r.role
                        FROM professeur pr
                        INNER JOIN roles r ON pr.id_encadrent = r.id_encadrent
                        WHERE pr.id_labos = $idLabos AND pr.id_encadrent != $idDirecteur";
                        $result = mysqli_query($connexion, $sql);
...
?>
    ...
    <div class="title text-center">
        ...
        <div class="projet">
    <form action="ajoute_projet.php" method="post">
        <label>Intitule</label>
        <input type="text" placeholder="Intitule" name="intitule">
        <label>Déscription</label>
        <input type="text" placeholder="Déscription" name="desc_projet"><br>
        <label>Durée</label>
        <input type="time" placeholder="Déscription" name="durée">
    ...
    <!-- Ajouter Projet End -->
    <h1><br><br><br></h1>
    <!--ajouter équipe activité start-->
    <div class="form-container">
        <h3>Ajouter équipe</h3>
    <form action="ajouter_équipe.php" method="post" enctype="multipart/form-data">
        <label for="nom">Équipe :</label>
        <input type="text" name="équipe" id="équipe" placeholder="Le nom d'équipe" required><br>
        <label for="image">Image d'équipe :</label>
        <input type="file" name="image" id="image" required><br>
        <label for="nombreAxes">Nombre d'axes :</label>
        <input type="number" id="nombreAxes" name="nombreAxes" min="1" required oninput="genererChampsAxes()"><br>
...
<script>
    function genererChampsAxes() {
        var nombreAxes = document.getElementById("nombreAxes").value;
        var axesContainer = document.getElementById("axesContainer");

        // Supprimer les champs d'axes précédemment générés
        axesContainer.innerHTML = "";

        // Générer les nouveaux champs d'axes
        for (var i = 1; i <= nombreAxes; i++) {
            var label = document.createElement("label");
            label.innerHTML = "Axe " + i + ":";
            ...
            input.name = "axes[]";
            input.required = true;
            axesContainer.appendChild(input);

            axesContainer.appendChild(document.createElement("br"));
        }
    }
</script>
