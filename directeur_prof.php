...
?>
    ...
    <div>
        ...
        WHERE d.id_encadrent = $idEncadrent";
...
if (isset($_POST["annee"])) {
    ...
    if (!empty($annee) && is_numeric($annee)) {
        ...
        if (mysqli_num_rows($resultatDoctoral) > 0) {
            ...
            echo "Aucun doctorant trouvé pour l'année $annee.";
        }
    } else {
    ...
    $sqlDoctoral = "SELECT * FROM doctoral WHERE id_encadrent =$idEncadrent";
        $resultatDoctoral = mysqli_query($connexion, $sqlDoctoral);
...
?>
    ...
    <section class="services">
    <div class="title text-center">
            <h1 class="title-blue"><a name="member" style="font-size: 60px;">Projets</a></h1>
        </div>
        <div>
        <?php
        $idEncadrent =  $_GET['id_encadrent'];  
        $sqlProf = "SELECT * FROM projet WHERE id_encadrent =$idEncadrent";
        ...
        if (mysqli_num_rows($resultatProf) > 0) {
            ...
            echo '<table>
                ...
                while ($rowProf = mysqli_fetch_assoc($resultatProf)) {
                    $Intitule = $rowProf['intitule'];
                    $description = $rowProf['desc_projet'];
                    $Date = $rowProf['durée'];
                    $Nature = $rowProf['nature'];
                    $Rapport = $rowProf['fichier'];
                    echo '<tr>
                        <td>' . $Intitule . '</td>
                        <td>' . $description . '</td>
                        <td>' . $Date . '</td>
                        <td>' . $Nature . '</td>
                        <td><a href="pdf_base/' . $Rapport . '">Voir le rapport</a></td>
                        </tr>';
